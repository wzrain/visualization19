...
struct Node{
	...
	double alt;
	Node(){}
	Node(vtkIdType i, double t){id=i;alt=t;}
};
...
std::unordered_set<int> prevActivate;
...
void getColorCorrespondingTovalue(double val, double& r, double& g, double& b, double range)
{
	...
	{
		178.0 / 255.0,24.0 / 255.0,43.0 / 255.0,
		214.0 / 255.0,96.0 / 255.0,77.0 / 255.0,
		244.0 / 255.0,165.0 / 255.0,130.0 / 255.0,
		253.0 / 255.0,219.0 / 255.0,199.0 / 255.0,
		255.0 / 255.0,255.0 / 255.0,255.0 / 255.0,
		209.0 / 255.0,229.0 / 255.0,240.0 / 255.0,
		146.0 / 255.0,197.0 / 255.0,222.0 / 255.0,
		67.0 / 255.0,147.0 / 255.0,195.0 / 255.0,
		33.0 / 255.0,102.0 / 255.0,172.0 / 255.0,
	};

	for (int i = 0; i < (numColorNodes - 1); i++)
	{
		double currFloor = -range + ((double)i / (numColorNodes - 1)) * 2 * range;
		double currCeil = -range + ((double)(i + 1) / (numColorNodes - 1)) * 2 * range;
...
void getColorCorrespondingTovalueRed(double val, double& r, double& g, double& b, double range)
{
	static const int numColorNodes = 9;
	double color[numColorNodes][3] =
	{
...
void getColorCorrespondingTovalueBlue(double val, double& r, double& g, double& b, double range)
{
	static const int numColorNodes = 9;
	double color[numColorNodes][3] =
	{
...
public:
	...
	virtual void Execute(vtkObject* caller, unsigned long, void*) {
		...
		for(std::unordered_set<int>::iterator itrs = prevActivate.begin(); itrs != prevActivate.end(); itrs++){
			newindex->SetValue(*itrs, 0);
		}
		prevActivate.clear();
		for(std::map<long,std::vector<Node>>::iterator itrm = timeLine.find(longnow); itrm != timeLine.upper_bound(longnow + timePath); itrm++){
			for(std::vector<Node>::iterator itrv = itrm->second.begin(); itrv != itrm->second.end(); itrv++){
				newindex->SetValue((*itrv).id,(*itrv).alt);
				prevActivate.insert((*itrv).id);
			}
		}
		data->GetPointData()->SetScalars(newindex);

		// vtkSmartPointer<vtkFloatArray> newindex = vtkSmartPointer<vtkFloatArray>::New();
		// newindex->SetNumberOfComponents(pointNumber);
		// for (int i = 0; i < data->GetNumberOfPoints(); i++){
		// 	double time = times->GetTuple1(i);
		// 	if(time >= longnow and time < longnow + timePath and pointBool[i]){
		// 		colors->SetValue(ix*gridSize + iy, s);
		// 		newindex->InsertNextValue(pointBool[i] * (altitude->GetTuple1(i)+2+));
		// 	}
		// 	else{
...
void readVTKFile()
{
	...
	clamsreader->SetFileName("../../../clams/CLaMS_Nabro.vtk");
	clamsreader->Update();
	...
	// vtkSmartPointer<vtkFloatArray> newindex = vtkSmartPointer<vtkFloatArray>::New();
	pointNumber = clamsdata->GetNumberOfPoints();
	...
	{
		double val = -range + ((double)i / numColors) * 2 * range;
	...
	clamsmapper->SetScalarRange(-range,range);
