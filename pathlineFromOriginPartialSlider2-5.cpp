...
public:
	...
	virtual void Execute(vtkObject* caller, unsigned long, void*) {
		...
		for (int i = 0; i < data->GetNumberOfPoints(); i++){
			...
			if(time >= longnow and time < longnow + timePath){
...
void readVTKFile()
{
	...
	for(int i = 0; i < trajs->GetNumberOfTuples(); i++){
		...
		for(int j = 0; j < pointNum; j++){
			...
			if(j == 0){
				if((std::abs(co[0]-NX) > delta or std::abs(co[1]-NY) > delta) and (std::abs(co[0]-GX) > delta or std::abs(co[1]-GY) > delta)){near=0;break;}
				else if((std::abs(co[0]-NX) <= delta or std::abs(co[1]-NY) <= delta)){near = 1;}
				else{near = -1;}
			}
	...
	for (int i = 0; i < numColors; i++)
	{
		...
		if(val == 0){std::cout << r << ' ' << g << ' ' << b << std::endl;}
